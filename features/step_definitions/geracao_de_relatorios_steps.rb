...
Given(/^o sistema possui "([^"]*)"kg de residuos cadastrados no laboratório de "([^"]*)"$/) do |res_weight, lab_name|
  ...
  res = lab.residues.create(name: "Acido", created_at: '23/02/2017'.to_date)
  reg = res.registers.create(weight: res_weight.to_f())
  #reg.residue_id = res.id
  #reg.save
  
  #res.registers.each do |d|
  #  p d + "------------------"
  #end
